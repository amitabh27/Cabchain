        ...
        implements NavigationView.OnNavigationItemSelectedListener,PlaceSelectionListener,OnMapReadyCallback,LocationListener,GoogleApiClient.ConnectionCallbacks,GoogleApiClient.OnConnectionFailedListener
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        double sourcelocationlatt=Double.parseDouble(GPSend.substring(0,GPSend.indexOf(" ")));
        double sourcelocationlongg=Double.parseDouble(GPSend.substring(GPSend.indexOf(" ")+1,GPSend.length()-1));


      /*  String myurl="https://maps.googleapis.com/maps/api/directions/json?origin="+sourcelocationlatt+","+sourcelocationlongg+"&destination="+latitude+","+longitude+"&sensor=false&mode=driving&alternatives=true&key=AIzaSyCweXwBZ82TU1ZdOCFoDFYhx9l75vh6E50";
        ...
        try{
            ...
            for(int i=0;i<steps.length();i++){
                current=steps.getJSONObject(i).getJSONObject("start_location");
                c=new LatLng(Double.parseDouble(current.getString("lat")),Double.parseDouble(current.getString("lng")));
                list.add(c);

                current=steps.getJSONObject(i).getJSONObject("end_location");
                c=new LatLng(Double.parseDouble(current.getString("lat")),Double.parseDouble(current.getString("lng")));
                list.add(c);
            }
...
*/
        ...
        h.append("Fare : "+fare+" Rs  Distance :"+distance+" km");
     ...
     /*   final Runnable r = new Runnable() {
            public void run() {
                ...
                String myUrl="https://cabchain.herokuapp.com/users/userredirection-finish/"+ridetrackingno;
                Log.d("URL",myUrl);
                HttpGetRequest getRequest = new HttpGetRequest();
                try{
                    String result = getRequest.execute(myUrl).get();
                    ...
                    if(r.equals("yes")){
                        Intent show = new Intent (getApplicationContext(), Ratings.class);
...
*/
    ...
    public void RateRide(View v){
        ...
        try{
            ...
            if(r.equals("yes")){
                ...
                Toast.makeText(this,"Your ride is still ongoing!",Toast.LENGTH_SHORT).show();
            }
        } catch(Exception e)
        {
            e.printStackTrace();
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Log.d("GPS START",GPSstart+"***");
        Log.d("GPS END",GPSend+"***");
        double latitude=Double.parseDouble(GPSstart.substring(0,GPSstart.indexOf(" ")));
        double longitude=Double.parseDouble(GPSstart.substring(GPSstart.indexOf(" ")+1,GPSstart.length()-1));
        double sourcelocationlatt=Double.parseDouble(GPSend.substring(0,GPSend.indexOf(" ")));
        double sourcelocationlongg=Double.parseDouble(GPSend.substring(GPSend.indexOf(" ")+1,GPSend.length()-1));
        ...
        mGoogleMap.addMarker(new MarkerOptions().position(dest)
                .title("Destination!").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_CYAN)));
